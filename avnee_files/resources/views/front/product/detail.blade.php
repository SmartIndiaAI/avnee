...
@php
    ...
    $promoPrimaryText = $settings['product_promo_primary_text'] ?? 'BUY USD 299 & GET 7% OFF';
    $promoPrimaryCode = $settings['product_promo_primary_code'] ?? 'SUMMER7';
    $promoSecondaryText = $settings['product_promo_secondary_text'] ?? 'BUY 2 OR MORE KIDS WEAR, GET EXTRA 20% OFF';
    $promoSecondaryCode = $settings['product_promo_secondary_code'] ?? 'KIDS20';
...
<div class="{{ $bgColor }} selection:bg-black selection:text-white transition-colors duration-700">
    <div class="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-10 pt-8 pb-20">
        ...
        @if(!$isDark)
        <section class="mb-6 border {{ $borderColor }} bg-[#b9dde0] px-4 py-3 text-center">
            <p class="text-[12px] font-semibold tracking-wide text-[#15343a]">Buy above ₹2,999 & get 7% off | Use Code: AVNEE7</p>
        </section>
        @endif
        ...
        <div class="grid grid-cols-1 xl:grid-cols-[72px_minmax(0,1fr)_360px] gap-5 xl:gap-7 items-start">
            ...
            <div class="order-3 {{ $textColor }} xl:sticky xl:top-20 {{ $isDark ? 'border border-[#4f006a] bg-[#230030]/55 p-4 sm:p-5' : '' }} {{ !$isDark ? 'bg-transparent' : '' }}">
                ...
                <div class="mb-6 space-y-2">
                    @if($isDark)
                    <div class="border border-[#9fd9d8] bg-[#d6f3f2] text-[#123538] px-5 py-3 text-center text-xs font-semibold tracking-wide">
                        {{ $promoPrimaryText }} | USE CODE: <span class="font-black tracking-wider">{{ $promoPrimaryCode }}</span>
                    </div>
                    @endif
                    <div class="border {{ $isDark ? 'border-[#e4ccd0] bg-[#f5e9eb] text-[#55363a]' : 'border-gray-300 bg-white text-gray-900' }} px-5 py-3 text-center text-[11px] font-semibold tracking-wide">
                        {{ $promoSecondaryText }} | USE CODE: <span class="font-black tracking-wider">{{ $promoSecondaryCode }}</span>
                    </div>
                </div>
                ...
                <form action="{{ route('cart.add') }}" method="POST" id="add-to-cart-form">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="size" value="{{ $variant->size }}" class="peer sr-only" {{ $index === 0 ? 'required' : '' }} />
                                <span class="inline-flex items-center justify-center min-w-[84px] h-9 px-3 rounded-full border {{ $borderColor }} text-xs font-medium peer-checked:bg-black peer-checked:text-white {{ $isDark ? 'peer-checked:bg-[#d4af37] peer-checked:text-[#1a0023]' : '' }}">
                                    {{ $variant->size }}
                                </span>
                            </label>
                            @endforeach
                    ...
                    <div class="space-y-3 mb-6 text-sm md:text-base {{ $mutedColor }} leading-snug">
                        <div class="flex items-center gap-2"><span class="text-base">◉</span><span>10 people are currently looking at this product.</span></div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                            ...
                            <div class="flex items-center gap-2"><span class="text-base">↻</span><span>5 Days easy returns*</span></div>
                        </div>
                    </div>
                </form>
                ...
                <div class="space-y-px border-t {{ $borderColor }} pt-2">
                    ...
                    <details class="group border-b {{ $borderColor }}">
                        ...
                        <div class="pb-5 text-[13px] leading-relaxed {{ $mutedColor }}">
                            Dummy text for style and fit guidance. Final content will be updated later.
                        </div>
                    </details>
                    ...
                    <details class="group border-b {{ $borderColor }}">
                        ...
                        <div class="pb-5 text-[13px] leading-relaxed {{ $mutedColor }}">
                            Dummy shipping and returns text placeholder. You can replace this with actual policy content later.
                        </div>
                    </details>
                    ...
                    <details class="group border-b {{ $borderColor }}">
                        ...
                        <div class="pb-5 text-[13px] leading-relaxed {{ $mutedColor }}">
                            Dummy FAQ text placeholder. Actual frequently asked questions can be added later.
                        </div>
                    </details>
