        ...
        <div class="swiper trending-now-swiper overflow-hidden px-1">
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide h-auto">
              <article class="bg-white border border-[#bdbdbd] p-2 sm:p-3 h-full flex flex-col relative">
                <a href="{{ $slide['link'] }}" class="block relative bg-[#f8f8f8] aspect-[3/4] overflow-hidden border border-[#d8d8d8]">
                  @if($slide['discount'] > 0)
                ...
                <div class="pt-4 px-2 pb-2 text-center flex-1 flex flex-col">
                  <a href="{{ $slide['link'] }}" class="block">
                    <h3 class="font-heading text-2xl sm:text-3xl leading-[1.12] text-[#272727] line-clamp-2 min-h-[3.8rem]">{{ $slide['name'] }}</h3>
                  </a>
                  <div class="mt-2 flex items-center justify-center gap-3 flex-wrap">
                    <span class="text-[#8a1f2f] text-3xl sm:text-4xl font-semibold">₹{{ number_format($slide['price'], 2) }}</span>
                    @if($slide['compare_price'] > $slide['price'])
                      <span class="text-[#444] text-2xl sm:text-3xl line-through">₹{{ number_format($slide['compare_price'], 2) }}</span>
                    @endif
                  </div>
                  @if($slide['discount'] > 0)
                    <p class="text-[#222] text-2xl sm:text-3xl mt-1">{{ $slide['discount'] }}% ({{ $slide['discount'] }}%)</p>
                  @endif
                  <a href="{{ $slide['link'] }}" class="mt-auto pt-4 inline-flex items-center justify-center w-full h-11 bg-[#1f1115] text-white text-lg sm:text-xl tracking-[0.08em] uppercase rounded-[3px]">Shop Now</a>
                </div>
              </article>
            </div>
            @endforeach
      ...
      <section id="shop-by-price" class="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 max-w-[1440px] mx-auto bg-white">
        ...
        <div class="flex flex-wrap justify-center gap-6 sm:gap-10 lg:gap-16">
          ...
          <a href="{{ $band['url'] }}" class="price-badge-wrap group relative flex items-center justify-center w-32 h-32 sm:w-40 sm:h-40 lg:w-48 lg:h-48">
            ...
            <div class="relative z-10 flex flex-col items-center justify-center text-center">
              <span class="font-heading italic text-[#F8C8DC]/80 text-[10px] sm:text-xs lg:text-base uppercase tracking-[0.2em] mb-0 sm:mb-1">{{ $band['label'] }}</span>
              <span class="font-heading text-2xl sm:text-3xl lg:text-4xl font-bold text-pastel-pink tracking-wider">₹{{ preg_replace('/[^0-9]/', '', (string) $band['price']) ?: $band['price'] }}</span>
            </div>
          </a>
          @endforeach
        </div>
