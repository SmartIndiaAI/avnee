...
<section id="customer-stories" class="py-16 sm:py-24 {{ $bgColor }} relative overflow-hidden transition-colors duration-700">
    ...
    <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="text-center mb-12 sm:mb-20">
            <h3 class="{{ $accentColor }} font-logo text-xl sm:text-2xl uppercase tracking-[0.4em] mb-4">Customer Stories</h3>
            <h2 class="font-heading text-2xl sm:text-4xl font-normal {{ $headingColor }} mb-12 tracking-[0.2em] uppercase {{ $isJewellery ? 'decoration-[#f3d9ff]/30' : 'decoration-mulberry/30' }} underline underline-offset-[12px]">
                {{ $isJewellery ? 'Cherished Moments...' : 'Loved by Little Princesses...' }}
            </h2>
            <div class="max-w-2xl mx-auto space-y-4">
                <p class="{{ $subtitleColor }} text-sm sm:text-base leading-relaxed uppercase tracking-wider font-bold">
                ...
                <p class="{{ $captionColor }} text-xs sm:text-sm leading-relaxed tracking-wide font-medium">
        ...
        <div class="relative group px-2 sm:px-12">
            ...
            <button id="stories-prev" class="absolute -left-2 sm:left-0 top-1/2 -translate-y-1/2 z-20 w-12 h-12 flex items-center justify-center {{ $accentColor }} hover:scale-125 transition-transform">
                <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M15 19l-7-7 7-7" /></svg>
            </button>
            <button id="stories-next" class="absolute -right-2 sm:right-0 top-1/2 -translate-y-1/2 z-20 w-12 h-12 flex items-center justify-center {{ $accentColor }} hover:scale-125 transition-transform">
                <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 5l7 7-7 7" /></svg>
            </button>
                    ...
                    <div class="swiper-slide h-auto self-stretch">
                        <div class="{{ $cardBg }} rounded-sm overflow-hidden flex flex-col h-full shadow-[0_20px_50px_rgba(0,0,0,0.05)] border group/card hover:shadow-2xl transition-all duration-700 backdrop-blur-sm">
                            ...
                            <div class="p-8 sm:p-10 flex flex-col items-center text-center flex-1">
                                <p class="{{ $isJewellery ? 'text-[#e9d5ff]/90' : 'text-gray-700' }} text-sm sm:text-[15px] leading-relaxed italic mb-8 flex-1 font-medium tracking-tight px-2 min-h-[110px]">
                                    {{ $review->comment }}
                                </p>

                                <div class="space-y-1 mb-8">
                                    <h4 class="{{ $accentColor }} font-bold text-lg tracking-tight uppercase leading-tight">{{ $review->user_name ?? $review->user?->name ?? 'AVNEE Customer' }}</h4>
                                    <p class="text-[10px] {{ $isJewellery ? 'text-[#e9d5ff]/40' : 'text-gray-400' }} font-bold uppercase tracking-[0.2em]">{{ $review->location ?? 'AVNEE Customer' }}</p>
                                </div>

                                @if($review->product)
                                <a href="{{ route('front.product.detail', $review->product->slug) }}" class="inline-block text-[10px] font-black uppercase tracking-[0.3em] {{ $isJewellery ? 'text-[#1a0023] bg-[#d4af37] hover:bg-white' : 'text-mulberry bg-mulberry/5 hover:bg-mulberry hover:text-white' }} px-6 py-2.5 rounded-sm transition-all">
                                    View Product
                                </a>
                                @endif
                            </div>
            ...
            <div class="flex justify-center gap-16 mt-12 sm:hidden relative z-20">
                <button id="m-stories-prev" class="{{ $accentColor }} p-2"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 19l-7-7 7-7" /></svg></button>
                <button id="m-stories-next" class="{{ $accentColor }} p-2"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5l7 7-7 7" /></svg></button>
            </div>
        </div>
    </div>
</section>
